...
void solution1(){
    ...
    printf("Заполните массив данными(Для препятствий 0, без препятствия 1, первый элемент поставьте 1)\n");
    for(int i=0;i<N;i++)
    {
        for(int j=0;j<M;j++)
            scanf("%i",&a[i][j]);
    ...
    printf("элемент(i,j) соответствует количеству маршрутов, по которым можно в этот элемент попасть.\nВаша карта количества маршрутов a(i,j):\n");
    for(int i=0;i<N;i++){
        for(int j=0;j<M;j++){
            printf("%i ",a[i][j]);
        }
...
void solution3(){
    printf("Введите размеры шахматной доски NxM\n");
    int N;
    printf("Введите N \n");
    scanf("%i",&N);
...
void searchSolution(int N, int M, int board[][M], int row[N*M], int col[N*M]){
    ...
    for ( a = 0 ; a <= 7 ; a++ ) {
        ...
        if (nomerHoda > (N*M)) {printBoard(N,M,board);exit(0);}
    ...
    ikon = row[nomerHoda]; jkon = col[nomerHoda];
    nomerHoda++;
